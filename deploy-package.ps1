...
.DESCRIPTION
    ...
    2. Packages clean production files into deploy.zip
    3. Excludes development bloat (node_modules, .git, .agents, tests, cache)
#>

Write-Host "=================================================" -ForegroundColor Cyan
Write-Host "  Budaya Tutur - Production Build Packager (budayatutur.id)" -ForegroundColor Cyan
Write-Host "=================================================" -ForegroundColor Cyan
...
$outputZip = Join-Path $PSScriptRoot "deploy.zip"
...
Write-Host "`n[3/3] Packaging files into deploy.zip..." -ForegroundColor Yellow
...
Write-Host "`nDeployment Steps for Hostinger (budayatutur.id):" -ForegroundColor Cyan
Write-Host "1. Upload 'deploy.zip' to the home directory where 'budaya-tutur' and 'public_html' reside."
Write-Host "2. Extract 'deploy.zip' (it will extract directly into 'budaya-tutur/' and 'public_html/')."
Write-Host "3. Delete 'deploy.zip' after extraction."
